...
export default function Login() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const draftPerfil = {
                ...
                alergias: alergiasTxt ? alergiasTxt.split(',').map(s => s.trim()).filter(Boolean) : null
            }
    ...
    // 🌈 Clase común para inputs en modo claro/oscuro
    const inputClass =
    ...
    return (
        <section className="section">
                    ...
                    {modo === 'registro' && (
                        <div className="grid gap-4 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input type="number" min="0" value={edad} onChange={(e) => setEdad(e.target.value)} className={inputClass} />
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" value={altura} onChange={(e) => setAltura(e.target.value)} className={inputClass} />
                            </div>

                            <div>
                                <label className="block text-sm mb-1">Peso (kg)</label>
                                <input type="number" step="0.01" value={peso} onChange={(e) => setPeso(e.target.value)} className={inputClass} />
                            </div>

                            <div>
                                <label className="block text-sm mb-1">Nivel de actividad</label>
                                <select value={nivelActividad} onChange={(e) => setNivelActividad(e.target.value)} className={inputClass}>
                            ...
                            <div>
                                ...
                                <div className="flex flex-col gap-2 text-sm">
                                    <label className="inline-flex items-center gap-2">
                                        <input type="checkbox" checked={prefVegetariano} onChange={(e) => setPrefVegetariano(e.target.checked)} />
                                        Vegetariano
                                    </label>
                                    <label className="inline-flex items-center gap-2">
                                        <input type="checkbox" checked={prefSinGluten} onChange={(e) => setPrefSinGluten(e.target.checked)} />
                                        Sin gluten
                                    </label>
                                </div>
                            </div>
                            ...
                            <div className="md:col-span-2">
                                ...
                                <input value={alergiasTxt} onChange={(e) => setAlergiasTxt(e.target.value)} className={inputClass} />
                            </div>
                        </div>
                    )}
            ...
            <Modal
                ...
                onClose={() => { setModalExisteOpen(false); setLoading(false) }}
                title="Este correo ya tiene una cuenta"
                actions={
                    <>
                        <button className="btn-ghost" onClick={() => setModalExisteOpen(false)}>Cerrar</button>
                        <button className="btn-primary" onClick={() => { setModo('login'); setModalExisteOpen(false) }}>Iniciar sesión</button>
                    </>
                }
            >
                <p>
            ...
            <Modal
                ...
                onClose={() => { setModalVerificacionOpen(false); setLoading(false) }}
                title="Confirma tu correo para continuar"
                actions={
                    <>
                        <button className="btn-ghost" onClick={() => setModalVerificacionOpen(false)}>Cerrar</button>
                        <button
            ...
            <Modal
                ...
                actions={
                    <>
                        <button className="btn-ghost" onClick={() => setModalResetOpen(false)}>Cancelar</button>
                        <button
                            className="btn-primary"
                            onClick={async () => {
                                setError('')
                                ...
                                try {
                                    ...
                                    setOk('Hemos enviado un correo para restablecer tu contraseña.')
                                    setModalResetOpen(false)
                                } catch (err) {
                                    setError(err.message || 'No se pudo enviar el correo. Inténtalo más tarde.')
                                }
                            }}
                        >
                            Enviar correo
