...
function extraerDias(plan, tipo) {
    ...
    // Caso 1: datos YA es un array de días
    if (Array.isArray(datos)) {
        console.log(`[Control] ${tipo}: datos es array de días`, datos)
        return datos
    }

    // Caso 2: datos.dias es un array (forma { nombre, dias: [...] })
    if (Array.isArray(datos.dias)) {
        console.log(`[Control] ${tipo}: usando datos.dias`, datos.dias)
        return datos.dias
    }

    // Caso 3: buscar la primera propiedad que sea array (por si viene distinto)
    const posibleArray = Object.values(datos).find(v => Array.isArray(v))
    if (Array.isArray(posibleArray)) {
        console.log(`[Control] ${tipo}: usando primer array encontrado en datos`, posibleArray)
        return posibleArray
    }

    console.warn('[Control] No se ha encontrado array de días en plan.datos:', datos)
    return null
}
    ...
    useEffect(() => {
        ...
        const cargarResumen = async () => {
            ...
            try {
                ...
                console.log('[Control] plan entrenamiento crudo:', planEnt)
                console.log('[Control] plan dieta crudo:', planDiet)

                const d = nombreDia

                // 🔍 extraer días de entrenamiento
                const diasEnt = extraerDias(planEnt, 'entrenamiento')
                const diaEntHoy = Array.isArray(diasEnt)
                    ? diasEnt.find(elem => coincideDiaPlan(elem.dia, d))
                    : null

                // 🔍 extraer días de dieta
                const diasDiet = extraerDias(planDiet, 'dieta')
                const diaDietHoy = Array.isArray(diasDiet)
                    ? diasDiet.find(elem => coincideDiaPlan(elem.dia, d))
                    : null

                console.log('[Control] Hoy entrenamiento:', diaEntHoy)
                console.log('[Control] Hoy dieta:', diaDietHoy)

                setHoyEntrenamiento(diaEntHoy || null)
                setHoyDieta(diaDietHoy || null)
            } catch (e) {
